...
export default function Links() {
    return (
        <m.div animate={{ opacity: 1, y: "0%" }} initial={{ opacity: 1, y: "150%" }} transition={{ duration: 1 }} exit={{ opacity: 0 }} className="h-full w-full flex flex-col absolute bg-one text-text">
            ...
            <div className="h-[60%] lg:h-[60%] md:h-[50%] md:mt-36 w-3/4 lg:space-y-8 lg:mt-6 my-20 lg:w-2/3 mx-auto no-scrollbar flex flex-col  overflow-scroll overscroll-none m-5" >
...
function SocialLink(props) {
    return (
        <div>
            {
                props.notalink == "true" ?
                    (<p className={`flex self-start w-full items-center left-0 ${props.side ? 'flex-row-reverse' : ''}  h-24 `}>
                        <SocialIcon style={{ height: 70, width: 70 }} network={props.network} />
                        <h2 className='my-auto sm:text-4xl ml-2 mr-2 lg:text-7xl'>{props.name}</h2>
                    </p>) :
